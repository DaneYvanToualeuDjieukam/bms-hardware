...
void setFeature2(bool CellBalanceDuringDischarge, bool CellbalanceDuringCharge,bool keepDFETonDuringCharge, bool keepCFETonDuringDischarge, bool shutdownOnUVLO, bool enableBalanceAtEOC);
...
byte readStatusReg(int regnum)
{
  byte stat;
  if(regnum == 1)
    return stat = readReg(0x80);
  else if(regnum == 2)
    return stat = readReg(0x81);
  else if(regnum == 3)
    return stat = readReg(0x82);
  else if(regnum == 4)
    return stat = readReg(0x82); 
  else
    Serial.print("read status bytes fail");
